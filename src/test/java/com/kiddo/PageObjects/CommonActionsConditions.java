...
public class CommonActionsConditions extends DesiredCapability {
	 ...
	 */
	public static WebElement waitForElementPresent(MobileElement webElement) {
		try {
			WebDriverWait wait = new WebDriverWait(driver, explicitWaitInSeconds);
			webElement = (MobileElement) wait.until(ExpectedConditions.visibilityOf(webElement));
	...
	public void ClickElement(WebElement accountStatement) {
...
//	}
	 ...
	 * @param driver
	 */
	public void scrollToText(String text, AndroidDriver<WebElement> driver)
...
//				topBottomScroll(driver, 0.80, 0.10);
	...
	public void scrollToLeft( AndroidDriver<WebElement> driver, String text) {
		
		MobileElement radioGroup = (MobileElement) driver.findElement(MobileBy.AndroidUIAutomator("new UiScrollable(new UiSelector().scrollable(true).instance(0)).setAsHorizontalList().scrollIntoView(new UiSelector().text(\""+text+"\"))"));
		sleepForMillis(2000);
		radioGroup.click();
